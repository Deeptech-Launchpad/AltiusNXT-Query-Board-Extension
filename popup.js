        ...
        card.innerHTML = `
            ...
            <div style="display:flex; justify-content:space-between; align-items: flex-start; margin-bottom:8px; padding-bottom:5px;">
                ...
                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 5px;">
                    ${batchHTML}
                    <table style="width: 250px; border-collapse: collapse; background: #fff; border: 1px solid #ddd; margin-left: auto;">
                        <thead style="background: #4b0082; color: white;">
                            <tr>
                                <th style="padding: 4px; font-size: 9px; border: 1px solid #ddd;">SKU ID</th>
                                <th style="padding: 4px; font-size: 9px; border: 1px solid #ddd;">Mfr.Part #</th>
                                <th style="padding: 4px; font-size: 9px; border: 1px solid #ddd;">Mfr. Name</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td style="padding: 4px; font-size: 9px; border: 1px solid #ddd; text-align: center; color: ${item.sku_id && item.sku_id !== 'nan' ? '#333' : '#999'};">${item.sku_id && item.sku_id !== 'nan' ? item.sku_id : 'Optional'}</td>
                                <td style="padding: 4px; font-size: 9px; border: 1px solid #ddd; text-align: center; color: ${item.mfr_part_number && item.mfr_part_number !== 'nan' ? '#333' : '#999'};">${item.mfr_part_number && item.mfr_part_number !== 'nan' ? item.mfr_part_number : 'Optional'}</td>
                                <td style="padding: 4px; font-size: 9px; border: 1px solid #ddd; text-align: center; color: ${item.manufacturer && item.manufacturer !== 'nan' ? '#333' : '#999'};">${item.manufacturer && item.manufacturer !== 'nan' ? item.manufacturer : 'Optional'}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        card.innerHTML = `
            ...
            <div style="display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:8px;">
                <div style="display:flex; align-items:center; gap:8px;">
                    <span style="background-color:${statusBg}; color:${statusColor}; padding:4px 10px; border-radius:12px; font-size:10px; font-weight:700;">${item.status.toUpperCase()}</span>
                </div>
                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 5px;">
                    ${batchHTML}
                    <table style="width: 250px; border-collapse: collapse; background: #fff; border: 1px solid #ddd; margin-left: auto;">
            ...
            ${attrDisplayHTML}
            <div class="result-query" style="font-size: 14px; color: #222;">Q: ${item.query}</div>
            ${answerHTML}`;
        resultsContainer.appendChild(card);
      } else {
