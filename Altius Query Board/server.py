...
def upload_schema():
    ...
    try:
        ...
        for _, row in df.iterrows():
            # Mapping based on your template
            p_name = str(row.get('Project Name', row.get('Project', ''))).strip()
            c_name = str(row.get('Taxonomy', row.get('Category', ''))).strip()
            a_name = str(row.get('Attribute', row.get('Attribute Name', ''))).strip()
            
            # Metadata for Knowledge Base
            uom = str(row.get('Units of Measure', '')).strip()
            defn = str(row.get('Definition', '')).strip()
            s_lov = str(row.get('Sample LOV', '')).strip()
            a_lov = str(row.get('Allowed LOV', '')).strip()
            d_type = str(row.get('Data Type', '')).strip()

            if not p_name or p_name == 'nan' or not c_name or c_name == 'nan': 
                continue

            # --- CRITICAL FIX: Add Project to the Master Projects Table ---
            # This ensures the project appears in the Extension Dropdown
            cur.execute("""
            ...
            # --- Sync details to known_attributes ---
            cur.execute("""
            ...
            """, (p_name, c_name, a_name, uom, defn, s_lov, a_lov, d_type))
        ...
        return jsonify({"status": "success", "message": f"Successfully synced {processed_count} attributes."})
...
def upload_kb_pdf():
    ...
    base_url = "https://qb.altiusnxt.com" 
    file_url = f"{base_url}/api/attachment/kb/{filename}"
...
def respond_query():
    ...
    if 'file' in request.files:
        ...
        if file and file.filename != '':
            ...
            attachment_url = f"https://qb.altiusnxt.com/api/attachment/{query_id}"
...
def trigger_startup_report():
    # We create a lock file in the temporary directory
    lock_file_path = '/tmp/query_board_report.lock'
    ...
    try:
        ...
        # LOCK_NB means "Non-Blocking". If another worker already has it, this fails immediately.
        fcntl.flock(f, fcntl.LOCK_EX | fcntl.LOCK_NB)
        
        # --- If we get here, this is the ONLY worker allowed to send the report ---
        print("🚀 [Worker ID: {}] Lock acquired. Sending Daily Report...".format(os.getpid()))
        
        try:
            send_daily_report()
            # Write the current time into the lock file for debugging
            f.write("Last sent at: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            f.flush()
        except Exception as e:
            print(f"⚠️ Startup Report Error: {e}")
            
        # IMPORTANT: We do NOT close the file or release the lock yet. 
        # We keep it locked as long as this worker process is alive.
        # This prevents other workers from sending it if they restart.

    except (IOError, OSError):
        # This means another worker process already grabbed the lock.
        print("ℹ️ [Worker ID: {}] Report already being handled. Skipping.".format(os.getpid()))

# Trigger the locked function
trigger_startup_report()
